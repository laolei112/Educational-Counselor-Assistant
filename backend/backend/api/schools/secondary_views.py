...
def serialize_secondary_school_for_list(school):
    ...
    if isinstance(transfer_info, dict):
        ...
        if s1_info and isinstance(s1_info, dict):
            start_str = s1_info.get('入学申请开始时间')
            end_str = s1_info.get('入学申请截至时间') or s1_info.get('入学申请截止时间')
            if start_str and end_str:
                try:
                    ...
                    if start <= now <= end:
                        ...
                        if days_left <= 7:
                            transfer_info_minimal['S1'] = {'application_status': 'deadline'}
                        else:
                            transfer_info_minimal['S1'] = {'application_status': 'open'}
                    elif now < start:
                        transfer_info_minimal['S1'] = {'application_status': 'closed'}
                    else:
                        transfer_info_minimal['S1'] = {'application_status': 'closed'}
                except:
                    # 如果解析失败，至少标记为有申请信息
